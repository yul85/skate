...
class SkateDartEnv(gym.Env):
    ...
    def step(self, _action):
        ...
        """
        action = np.hstack((np.zeros(6), np.asarray(_action)/10.))
    ...
    def reset(self):
        ...
        q = np.asarray(self.ref_motion.qs[self.phase_frame])
        dq = np.asarray(self.ref_motion.dqs[self.phase_frame])

        if self.add_noise:
            q = np.random.normal(q, self.noise_mag)
            dq = np.random.normal(dq, self.noise_mag)
