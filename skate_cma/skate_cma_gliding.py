...
def objective(i, penalty_option0, penalty_option1, penalty_weight):
    ...
    penalty_option0[PenaltyType.COM_VEL] = np.array([0.8, 0.0, 0.0])
    penalty_option1[PenaltyType.COM_VEL] = np.array([0.8, 0.0, 0.0])
...
if __name__ == '__main__':
    pydart.init()
    if len(sys.argv) == 1:
        cma = HpCma('hmr_skating_gliding', 24, sigma=0.1, max_time=4.5, cma_timeout=3600)
        ...
        cma = HpCma('hmr_skating_gliding', 1)
