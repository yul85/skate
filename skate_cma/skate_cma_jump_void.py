...
def objective(i, penalty_option0, penalty_option1, penalty_weight):
    ...
    penalty_weight[PenaltyType.REGULARIZE] = 0.
    penalty_weight[PenaltyType.TORQUE] = 1e-6
...
if __name__ == '__main__':
    ...
    pydart.init()
    if len(sys.argv) == 1:
        cma = HpCma('jump_void', 4, sigma=0.1, max_time=2., cma_timeout=1)
        ...
        cma = HpCma('jump_void', 1)

    with open('jump0507_2.skkey', 'rb') as skkey_file:
        skkey_states = pickle.load(skkey_file)
    dq = np.zeros(57)
    dq[3] = -1.5
    cma.set_init_dq(dq)
    cma.set_init_q(skkey_states[0].angles)
