...
class SkateDartEnv(gym.Env):
    def __init__(self, ):
        ...
        self.ref_motion.load(cur_path+'/jump_back.skmo')
        self.ref_motion.set_avg_x_vel(-2.*random())
    ...
    def is_done(self):
        if self.skel.com()[1] < 0.2:
            ...
            return True
        elif self.skel.body('h_hand_left') in self.world.collision_result.contacted_bodies:
            return True
        elif self.skel.body('h_hand_right') in self.world.collision_result.contacted_bodies:
            return True
    ...
    def step(self, _action):
        ...
        for i in range(self.step_per_frame):
            if self.ext_force_duration > 0.:
                self.skel.body('h_pelvis').add_ext_force(self.ext_force, _isForceLocal=True)
    ...
    def reset_with_q_dq(self, q, dq):
        self.world.reset()
        xz_vel = np.array([dq[3], 0., dq[5]])
        self.ref_motion.set_avg_x_vel(-np.linalg.norm(xz_vel))
        self.ref_motion.refine_dqs(self.ref_skel)
